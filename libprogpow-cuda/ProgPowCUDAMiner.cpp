...
#include "ProgPowCUDAMiner.h"
#include "ProgPowCUDAMiner_kernel.h"
#include <nvrtc.h>
...
ProgPowCUDAMiner::~ProgPowCUDAMiner()
{
	stopWorking();
	kick_miner();
}
...
bool ProgPowCUDAMiner::init(int epoch)
{
	try {
...
uint64_t get_start_nonce(uint64_t _index)
{
	...
	try
	{
		while(!shouldStop())
		{
			...
			if (current.header != w.header || current.epoch != w.epoch || old_period_seed != period_seed)
			{
				...
				{
					uint64_t dagBytes = ethash_get_datasize(w.block);
					uint32_t dagElms   = (unsigned)(dagBytes / (PROGPOW_LANES * PROGPOW_DAG_LOADS * 4));
					compileKernel(w.block, dagElms);
				}
...
void ProgPowCUDAMiner::kick_miner()
{
	m_new_work.store(true, std::memory_order_relaxed);
}

void ProgPowCUDAMiner::setNumInstances(unsigned _instances)
{
...
void ProgPowCUDAMiner::listDevices()
{
	try
	{
...
bool ProgPowCUDAMiner::configureGPU(
	unsigned _blockSize,
	unsigned _gridSize,
...
void ProgPowCUDAMiner::setParallelHash(unsigned _parallelHash)
{
  	m_parallelHash = _parallelHash;
}

unsigned const ProgPowCUDAMiner::c_defaultBlockSize = 512;
unsigned const ProgPowCUDAMiner::c_defaultGridSize = 1024; // * CL_DEFAULT_LOCAL_WORK_SIZE
unsigned const ProgPowCUDAMiner::c_defaultNumStreams = 2;

bool ProgPowCUDAMiner::cuda_configureGPU(
	size_t numDevices,
...
unsigned ProgPowCUDAMiner::m_parallelHash = 4;
unsigned ProgPowCUDAMiner::s_blockSize = ProgPowCUDAMiner::c_defaultBlockSize;
unsigned ProgPowCUDAMiner::s_gridSize = ProgPowCUDAMiner::c_defaultGridSize;
unsigned ProgPowCUDAMiner::s_numStreams = ProgPowCUDAMiner::c_defaultNumStreams;
unsigned ProgPowCUDAMiner::s_scheduleFlag = 0;
bool ProgPowCUDAMiner::s_noeval = false;

bool ProgPowCUDAMiner::cuda_init(
	size_t numDevices,
...
void ProgPowCUDAMiner::compileKernel(
	...
	uint64_t dag_elms)
{
	...
	text += std::string(ProgPowCUDAMiner_kernel, sizeof(ProgPowCUDAMiner_kernel));
		...
		if (m_current_index >= s_numStreams)
		{
            if (found_count)
            {
                for (uint32_t i = 0; i < found_count; i++)
                    if (s_noeval)
                        Farm::f().submitProof(Solution{nonces[i], mixes[i], w, std::chrono::steady_clock::now(),(unsigned int)m_current_index});
                    else
                    {
                        Result r = ProgPowAux::eval(w.epoch, w.header, nonces[i]);
                        if (r.value < w.boundary)
                            Farm::f().submitProof(Solution{nonces[i], mixes[i], w, std::chrono::steady_clock::now(),(unsigned int)m_current_index});
                        else
                        {
                            // Farm::f().failedSolution();
                            cwarn << "GPU gave incorrect result!";
                        }
                    }
            }

         	updateHashRate(batch_size, m_settings.streams);
